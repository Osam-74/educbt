...
The existing report route displayed a staff-only notice but did not enforce publication or student/guardian ownership. Its read path now enforces school-wide staff access, assigned class-teacher access from reviewed onward, and published/locked results for the student's own account or a linked guardian. References: `templates/portal/student/results.php`, `templates/portal/guardian/results.php`, `templates/portal/teacher/report.php`. This security correction is required for publication to mean what the dashboard promises; it does not build a new student or parent portal.
