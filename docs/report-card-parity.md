...
Print continuation verification: the screen preview retains three tiles, but print uses two tiles at 60mm and 166mm within the 277mm printable height. The off-page third fixed tile caused Chromium to create a blank watermark-only page. Linux CI now asserts the actual short report PDF has exactly one page. The original crest-presence checks still inspect the full report; the second-tile/gap probes use an identically paginated copy with foreground ink hidden so table text cannot falsely fail the gap threshold. No threshold is relaxed.
