...
async function main() {
  ...
  try {
    ...
    // Owner decision (2026-09-15): report-card remarks are customised by
    // CLASS TEACHERS at result approval — the principal account never edits
    // automatic remark ranges.
    await check('principal cannot save remark ranges', () => assert.rejects(service.saveRanges(principal, 'principal', [{ min: 0, remark: 'Keep working steadily.' }]), /class teacher role only/));
    await check('exam officer cannot save remark ranges', () => assert.rejects(service.saveRanges(a.actors.exam_officer!, 'exam_officer', [{ min: 0, remark: 'Nope.' }])));
    ...
    await check('automatic zero-average remark is snapshotted', async () => { await forSchool(a.schoolId, tx => snapshotRemarks(tx, principal, a.scope, [a.student.id])); const extras = await forSchool(a.schoolId, tx => reportExtras(tx, a.schoolId, a.student.id, a.session.id, a.terms[0]!.id, a.classroom.id)); assert.equal(extras.remarks.principal, 'Poor performance. Must show significant improvement next term.', 'no principal ranges exist — the default remark applies'); assert(extras.signatures.principal); assert(extras.signatures.class_teacher); });
    await check('manual remark survives compilation snapshot', async () => { await saveManualRemark(principal, remarkScope); await forSchool(a.schoolId, tx => snapshotRemarks(tx, principal, a.scope, [a.student.id])); const extras = await forSchool(a.schoolId, tx => reportExtras(tx, a.schoolId, a.student.id, a.session.id, a.terms[0]!.id, a.classroom.id)); assert.equal(extras.remarks.principal, remarkScope.remark); });
